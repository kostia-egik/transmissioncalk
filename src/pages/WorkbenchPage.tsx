...
import { ERROR_BG_COLOR, ERROR_TEXT_COLOR, getRotationIconPath, PLANETARY_SHAFT_INPUT_BG, GEAR_MODULES, CHAIN_PITCHES, TOOTHED_BELT_PITCHES } from '../constants';
...
const GearModuleDisplay: React.FC<Pick<ModuleProps, 'moduleData' | 'stageIndex' | 'moduleIndex' | 'onInputChange' | 'isExpanded' | 'onParamClick'>> = ({ moduleData, stageIndex, moduleIndex, onInputChange, isExpanded, onParamClick }) => { const inputs = moduleData.inputs as GearInputParams; const calculated = { "Межосевое a": moduleData.a, "Диаметр d₁": moduleData.d1, "Диаметр d₂": moduleData.d2, "d вершин dₐ₁": moduleData.da1, "d вершин dₐ₂": moduleData.da2, "d впадин d𝒻₁": moduleData.df1, "d впадин d𝒻₂": moduleData.df2, }; const renderToDataKeyMap = { "Межосевое a": "a", "Диаметр d₁": "d1", "Диаметр d₂": "d2", "d вершин dₐ₁": "da1", "d вершин dₐ₂": "da2", "d впадин d𝒻₁": "df1", "d впадин d𝒻₂": "df2" }; return (<div onClick={e => e.stopPropagation()}><div className="grid grid-cols-2 gap-2"><InputWithControls onLabelClick={(t) => onParamClick('z1', t)} label="z₁ (ведущая)" value={inputs.z1} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z1', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z1, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v))} onDecrement={() => handleNumericInputWithControls(inputs.z1, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v), 0)} inputClassName={`text-gray-800`}/> <InputWithControls onLabelClick={(t) => onParamClick('z2', t)} label="z₂ (ведомая)" value={inputs.z2} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z2', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z2, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v))} onDecrement={() => handleNumericInputWithControls(inputs.z2, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v), 0)} inputClassName={`text-gray-800`}/></div> {isExpanded && <div className="pt-2 border-t mt-2 space-y-1"><div className="max-w-24"><SelectOrInput min={0} onLabelClick={(t) => onParamClick('m', t)} label="Модуль, m" options={GEAR_MODULES} value={inputs.m} onChange={(v) => onInputChange(stageIndex, moduleIndex, 'm', v)} inputClassName={`text-gray-800`} selectClassName={`text-gray-800`} /></div> <Input onLabelClick={(t) => onParamClick('eta', t)} label="КПД (0.0-1.0)" value={inputs.eta ?? '0.98'} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'eta', e.target.value)} type="number" min={0} max={1} step={0.01} placeholder="0.98" inputClassName={`text-gray-800`} className="!mb-1 max-w-24" /><CalculatedParamsDisplay params={calculated} onParamClick={onParamClick} renderToDataKeyMap={renderToDataKeyMap}/></div>}</div>);};
const ChainModuleDisplay: React.FC<Pick<ModuleProps, 'moduleData' | 'stageIndex' | 'moduleIndex' | 'onInputChange' | 'isExpanded' | 'onParamClick'>> = ({ moduleData, stageIndex, moduleIndex, onInputChange, isExpanded, onParamClick }) => { const inputs = moduleData.inputs as ChainInputParams; const calculated = { "Делит. d₁": moduleData.chain_d1, "Делит. d₂": moduleData.chain_d2, "Внешн. da₁": moduleData.chain_da1, "Внешн. da₂": moduleData.chain_da2, "Мин. межосевое aₘᵢₙ": moduleData.chain_amin, }; const warnings = { "Мин. межосевое aₘᵢₙ": "ВНИМАНИЕ: Это теор. расстояние по делит. диаметрам. Реальное (по внешним диаметрам + зазор) будет больше." }; const renderToDataKeyMap = { "Делит. d₁": "chain_d1", "Делит. d₂": "chain_d2", "Внешн. da₁": "chain_da1", "Внешн. da₂": "chain_da2", "Мин. межосевое aₘᵢₙ": "chain_amin" }; return (<div onClick={e => e.stopPropagation()}><div className="grid grid-cols-2 gap-2"><InputWithControls onLabelClick={(t) => onParamClick('z1', t)} label="z₁ (ведущая)" value={inputs.z1} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z1', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z1, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v))} onDecrement={() => handleNumericInputWithControls(inputs.z1, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v), 0)} inputClassName={`text-gray-800`} /> <InputWithControls onLabelClick={(t) => onParamClick('z2', t)} label="z₂ (ведомая)" value={inputs.z2} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z2', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z2, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v))} onDecrement={() => handleNumericInputWithControls(inputs.z2, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v), 0)} inputClassName={`text-gray-800`} /></div> {isExpanded && <div className="pt-2 border-t mt-2 space-y-1"><div className="max-w-24"><SelectOrInput min={0} onLabelClick={(t) => onParamClick('p', t)} label="Шаг цепи, p (мм)" options={CHAIN_PITCHES} value={inputs.p} onChange={(v) => onInputChange(stageIndex, moduleIndex, 'p', v)} inputClassName={`text-gray-800`} selectClassName={`text-gray-800`} /></div> <Input onLabelClick={(t) => onParamClick('eta', t)} label="КПД (0.0-1.0)" value={inputs.eta ?? '0.95'} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'eta', e.target.value)} type="number" min={0} max={1} step={0.01} placeholder="0.95" inputClassName={`text-gray-800`} className="!mb-1 max-w-24" /><CalculatedParamsDisplay params={calculated} warnings={warnings} onParamClick={onParamClick} renderToDataKeyMap={renderToDataKeyMap}/></div>}</div>);};
const PlanetaryModuleDisplay: React.FC<Pick<ModuleProps, 'moduleData' | 'stageIndex' | 'moduleIndex' | 'onInputChange' | 'onSelectChange' | 'isExpanded' | 'onParamClick'>> = ({ moduleData, stageIndex, moduleIndex, onInputChange, onSelectChange, isExpanded, onParamClick }) => { 
    const inputs = moduleData.inputs as PlanetaryInputParams;
    const assemblyText = moduleData.assemblyPossible ? "Возможна" : (moduleData.assemblyError || (moduleData.zPlanet !== undefined ? "СБОРКА НЕВОЗМОЖНА" : "")); 
    const calculated = { "z Сателлита": moduleData.zPlanet, "Условие сборки": assemblyText, "d Солнца": moduleData.planetary_dSun, "d Сателлита": moduleData.planetary_dPlanet, "d Короны": moduleData.planetary_dRing, "Межосевое a (S-P)": moduleData.planetary_a, "Зафиксир. вал": moduleData.fixedShaft, }; 
    const renderToDataKeyMap = { "z Сателлита": "zPlanet", "Условие сборки": "assemblyPossible", "d Солнца": "planetary_dSun", "d Сателлита": "planetary_dPlanet", "d Короны": "planetary_dRing", "Межосевое a (S-P)": "planetary_a", "Зафиксир. вал": "fixedShaft" };
    ...
    return (<div onClick={e => e.stopPropagation()}><div className="grid grid-cols-2 gap-2"><InputWithControls onLabelClick={(t) => onParamClick('zSun', t)} label="z Солнца" value={inputs.zSun} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'zSun', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.zSun, 1, (v) => onInputChange(stageIndex, moduleIndex, 'zSun', v))} onDecrement={() => handleNumericInputWithControls(inputs.zSun, -1, (v) => onInputChange(stageIndex, moduleIndex, 'zSun', v), 0)} inputClassName={`text-gray-800`} /> <InputWithControls onLabelClick={(t) => onParamClick('zRing', t)} label="z Короны" value={inputs.zRing} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'zRing', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.zRing, 1, (v) => onInputChange(stageIndex, moduleIndex, 'zRing', v))} onDecrement={() => handleNumericInputWithControls(inputs.zRing, -1, (v) => onInputChange(stageIndex, moduleIndex, 'zRing', v), 0)} inputClassName={`text-gray-800`} /></div> <CustomSelect onLabelClick={(t) => onParamClick('shaftConfig', t)} label="Вход → Выход" value={inputs.shaftConfig || ""} onChange={(val) => onSelectChange?.(stageIndex, moduleIndex, 'shaftConfig', val as PlanetaryConfig)} options={customSelectOptions} emptyOptionLabel="-- Выбрать --" className="!mb-1" selectClassName={`${PLANETARY_SHAFT_INPUT_BG} text-gray-800 !py-1 !text-sm`} /> {isExpanded && <div className="pt-2 border-t mt-2 space-y-1"><div className="max-w-24"><SelectOrInput min={0} onLabelClick={(t) => onParamClick('m', t)} label="Модуль, m" options={GEAR_MODULES} value={inputs.m} onChange={(v) => onInputChange(stageIndex, moduleIndex, 'm', v)} inputClassName={`text-gray-800`} selectClassName={`text-gray-800`} /></div> <Input onLabelClick={(t) => onParamClick('eta', t)} label="КПД (0.0-1.0)" value={inputs.eta ?? '0.98'} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'eta', e.target.value)} type="number" min={0} max={1} step={0.01} placeholder="0.98" inputClassName={`text-gray-800`} className="!mb-1 max-w-24" /><CalculatedParamsDisplay params={calculated} onParamClick={onParamClick} renderToDataKeyMap={renderToDataKeyMap}/></div>}</div>);
};
const ToothedBeltModuleDisplay: React.FC<Pick<ModuleProps, 'moduleData' | 'stageIndex' | 'moduleIndex' | 'onInputChange' | 'isExpanded' | 'onParamClick'>> = ({ moduleData, stageIndex, moduleIndex, onInputChange, isExpanded, onParamClick }) => { const inputs = moduleData.inputs as ToothedBeltInputParams; const calculated = { "d₁ (шкив)": moduleData.tb_d1, "d₂ (шкив)": moduleData.tb_d2, "aₘᵢₙ (теор.)": moduleData.tb_amin, }; const warnings = { "aₘᵢₙ (теор.)": "ВНИМАНИЕ: Это теоретическое расстояние по делительным диаметрам шкивов. Реальное межосевое расстояние будет зависеть от выбранного ремня и требуемого натяжения." }; const renderToDataKeyMap = { "d₁ (шкив)": "tb_d1", "d₂ (шкив)": "tb_d2", "aₘᵢₙ (теор.)": "tb_amin" }; return (<div onClick={e => e.stopPropagation()}><div className="grid grid-cols-2 gap-2"><InputWithControls onLabelClick={(t) => onParamClick('z1', t)} label="z₁ (ведущий)" value={inputs.z1} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z1', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z1, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v))} onDecrement={() => handleNumericInputWithControls(inputs.z1, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v), 0)} inputClassName={`text-gray-800`} /> <InputWithControls onLabelClick={(t) => onParamClick('z2', t)} label="z₂ (ведомый)" value={inputs.z2} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z2', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z2, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v))} onDecrement={() => handleNumericInputWithControls(inputs.z2, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v), 0)} inputClassName={`text-gray-800`} /></div> {isExpanded && <div className="pt-2 border-t mt-2 space-y-1"><div className="max-w-24"><SelectOrInput min={0} onLabelClick={(t) => onParamClick('p', t)} label="Шаг ремня, p (мм)" options={TOOTHED_BELT_PITCHES} value={inputs.p} onChange={(v) => onInputChange(stageIndex, moduleIndex, 'p', v)} inputClassName={`text-gray-800`} selectClassName={`text-gray-800`} /></div> <Input onLabelClick={(t) => onParamClick('eta', t)} label="КПД (0.0-1.0)" value={inputs.eta ?? '0.95'} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'eta', e.target.value)} type="number" min={0} max={1} step={0.01} placeholder="0.95" inputClassName={`text-gray-800`} className="!mb-1 max-w-24" /><CalculatedParamsDisplay params={calculated} warnings={warnings} onParamClick={onParamClick} renderToDataKeyMap={renderToDataKeyMap}/></div>}</div>);};
const BeltModuleDisplay: React.FC<Pick<ModuleProps, 'moduleData' | 'stageIndex' | 'moduleIndex' | 'onInputChange' | 'isExpanded' | 'onParamClick'>> = ({ moduleData, stageIndex, moduleIndex, onInputChange, isExpanded, onParamClick }) => { 
    ...
    const warnings = { "aₘᵢₙ (теор.)": "ВНИМАНИЕ: Это теоретическое расстояние по расчетным диаметрам шкивов. Реальное межосевое расстояние будет зависеть от типа ремня, его длины и требуемого натяжения." }; 
    ...
    return (
        <div onClick={e => e.stopPropagation()}>
            ...
            {isExpanded && <div className="pt-2 border-t mt-2 space-y-1"><Input onLabelClick={(t) => onParamClick('eta', t)} label="КПД (0.0-1.0)" value={inputs.eta ?? '0.95'} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'eta', e.target.value)} type="number" min={0} max={1} step={0.01} placeholder="0.95" inputClassName={`text-gray-800`} className="!mb-1 max-w-24" /><CalculatedParamsDisplay params={calculated} warnings={warnings} onParamClick={onParamClick} renderToDataKeyMap={renderToDataKeyMap}/></div>}
        </div>
    );
};
const BevelGearModuleDisplay: React.FC<Pick<ModuleProps, 'moduleData' | 'stageIndex' | 'moduleIndex' | 'onInputChange' | 'onSelectChange' | 'isExpanded' | 'onParamClick'>> = ({ moduleData, stageIndex, moduleIndex, onInputChange, onSelectChange, isExpanded, onParamClick }) => { 
    const inputs = moduleData.inputs as BevelGearInputParams; 
    const calculated = { "Делит. d₁": moduleData.bevel_d1, "Делит. d₂": moduleData.bevel_d2, "Угол δ₁ (°)": moduleData.bevel_delta1, "Угол δ₂ (°)": moduleData.bevel_delta2, "Внеш. конус. расст. Re": moduleData.bevel_Re, "Средний dₘ₁": moduleData.bevel_dm1, "Средний dₘ₂": moduleData.bevel_dm2, }; 
    const renderToDataKeyMap = { "Делит. d₁": "bevel_d1", "Делит. d₂": "bevel_d2", "Угол δ₁ (°)": "bevel_delta1", "Угол δ₂ (°)": "bevel_delta2", "Внеш. конус. расст. Re": "bevel_Re", "Средний dₘ₁": "bevel_dm1", "Средний dₘ₂": "bevel_dm2" };
    ...
    return (<div onClick={e => e.stopPropagation()}><div className="grid grid-cols-2 gap-2"><InputWithControls onLabelClick={(t) => onParamClick('z1', t)} label="z₁ (ведущая)" value={inputs.z1} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z1', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z1, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v))} onDecrement={() => handleNumericInputWithControls(inputs.z1, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v), 0)} inputClassName={`text-gray-800`} /> <InputWithControls onLabelClick={(t) => onParamClick('z2', t)} label="z₂ (ведомая)" value={inputs.z2} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z2', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z2, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v))} onDecrement={() => handleNumericInputWithControls(inputs.z2, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v), 0)} inputClassName={`text-gray-800`} /></div> <div className="!mb-1"><CustomSelect onLabelClick={(t) => onParamClick('config_bevel', t)} label="Тип" value={inputs.config || ""} onChange={(val) => onSelectChange?.(stageIndex, moduleIndex, 'config', val as BevelGearConfigType)} options={customSelectOptions} emptyOptionLabel="-- Выбрать --" selectClassName={`text-gray-800 !py-1 !text-sm`} /></div>{isExpanded && <div className="pt-2 border-t mt-2 space-y-1"><div className="max-w-24"><SelectOrInput min={0} onLabelClick={(t) => onParamClick('m', t)} label="Модуль, m" options={GEAR_MODULES} value={inputs.m} onChange={(v) => onInputChange(stageIndex, moduleIndex, 'm', v)} inputClassName={`text-gray-800`} selectClassName={`text-gray-800`} /></div> <InputWithControls onLabelClick={(t) => onParamClick('b', t)} label="Ширина венца, b" value={inputs.b} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'b', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.b, 1, (v) => onInputChange(stageIndex, moduleIndex, 'b', v))} onDecrement={() => handleNumericInputWithControls(inputs.b, -1, (v) => onInputChange(stageIndex, moduleIndex, 'b', v), 0)} inputClassName={`text-gray-800`} /> <Input onLabelClick={(t) => onParamClick('eta', t)} label="КПД (0.0-1.0)" value={inputs.eta ?? '0.98'} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'eta', e.target.value)} type="number" min={0} max={1} step={0.01} placeholder="0.98" inputClassName={`text-gray-800`} className="!mb-1 max-w-24" /><CalculatedParamsDisplay params={calculated} onParamClick={onParamClick} renderToDataKeyMap={renderToDataKeyMap}/></div>}</div>);
};
const WormGearModuleDisplay: React.FC<Pick<ModuleProps, 'moduleData' | 'stageIndex' | 'moduleIndex' | 'onInputChange' | 'onSelectChange' | 'isExpanded' | 'onParamClick'>> = ({ moduleData, stageIndex, moduleIndex, onInputChange, onSelectChange, isExpanded, onParamClick }) => { 
    ...
    return (<div onClick={e => e.stopPropagation()}><div className="grid grid-cols-2 gap-2"><InputWithControls onLabelClick={(t) => onParamClick('z1', t)} label="z₁ (червяк)" value={inputs.z1} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z1', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z1, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v))} onDecrement={() => handleNumericInputWithControls(inputs.z1, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z1', v), 0)} inputClassName={`text-gray-800`} /> <InputWithControls onLabelClick={(t) => onParamClick('z2', t)} label="z₂ (колесо)" value={inputs.z2} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'z2', e.target.value)} onIncrement={() => handleNumericInputWithControls(inputs.z2, 1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v))} onDecrement={() => handleNumericInputWithControls(inputs.z2, -1, (v) => onInputChange(stageIndex, moduleIndex, 'z2', v), 0)} inputClassName={`text-gray-800`} /></div> <div className="!mb-1"><CustomSelect onLabelClick={(t) => onParamClick('config_worm', t)} label="Тип" value={inputs.config || ""} onChange={(val) => onSelectChange?.(stageIndex, moduleIndex, 'config', val as WormGearConfigType)} options={customSelectOptions} emptyOptionLabel="-- Выбрать --" selectClassName={`text-gray-800 !py-1 !text-sm`} /></div>{isExpanded && <div className="pt-2 border-t mt-2 space-y-1"><div className="max-w-24"><SelectOrInput min={0} onLabelClick={(t) => onParamClick('m', t)} label="Модуль, m" options={GEAR_MODULES} value={inputs.m} onChange={(v) => onInputChange(stageIndex, moduleIndex, 'm', v)} inputClassName={`text-gray-800`} selectClassName={`text-gray-800`} /></div> <Input min={0} onLabelClick={(t) => onParamClick('q', t)} label="Коэф. диам. q" value={inputs.q} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'q', e.target.value)} type="number" inputClassName={`text-gray-800`} className="!mb-1 max-w-24" /> <Input onLabelClick={(t) => onParamClick('eta', t)} label="КПД (0.3-0.9)" value={inputs.eta ?? '0.7'} onChange={(e) => onInputChange(stageIndex, moduleIndex, 'eta', e.target.value)} type="number" min={0} max={1} step={0.01} placeholder="0.7" inputClassName={`text-gray-800`} className="!mb-1 max-w-24" /><CalculatedParamsDisplay params={calculated} onParamClick={onParamClick} renderToDataKeyMap={renderToDataKeyMap}/></div>}</div>);
};
  ...
  calculationDataSnapshot, showNotification, finalResultsRef, setGlobalError, scrollToModuleId, onScrollComplete
    ...
    const removeStage = (stageIndex: number) => {
        const newData = calculationData.filter((_, i) => i !== stageIndex)
            .map((stage, i) => ({ ...stage, stageName: `Валы ${i + 1} и ${i + 2}` }));
    ...
    const handleGearTypeChange = (stageIndex: number, moduleIndex: number, newType: GearType) => {
        ...
        switch (newType) {
            ...
            case GearType.Bevel: defaultInputs = { z1: '', z2: '', m: '1', b: '10', config: BEVEL_GEAR_CONFIG_OPTIONS[0].value, eta: '0.98' }; break;
    ...
    const handleInputChange = (stageIndex: number, moduleIndex: number, field: string, value: string | number) => {
        ...
        const moduleInputs = updatedData[stageIndex].modules[moduleIndex].inputs as any;
        moduleInputs[field] = value;
        ...
        <div className="space-y-8 py-8">
            ...
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg">
                ...
                <div className="mt-4 flex justify-start">
                    <Button onClick={() => { resetEngineParams(); setGlobalError(null); setErrors({}); }} variant="secondary" className="text-sm px-3 py-1">Сбросить</Button>
                </div>
            </div>
            ...
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg">
                ...
                >
                    {calculationData.map((stage, stageIndex) => {
                        ...
                        <div key={stage.id}>
                            <div className="flex justify-between items-center mb-4">
                                ...
                                {calculationData.length > 1 && <Button onClick={() => removeStage(stageIndex)} variant="secondary" className="!p-2 text-sm leading-none" title={`Удалить ступень ${stageIndex + 1}`}><CrossIcon /></Button>}
                            </div>
                            ...
                            <div className="overflow-x-auto -mx-4 sm:-mx-6">
                                ...
                                {stage.modules.map((moduleData, moduleIndex) => {
                                    ...
                                    if (moduleData.id === highlightedModuleId) {
                                        highlightClasses = 'shadow-[0_0_6px_4px_rgba(251,146,60,0.7)]';
                                    } else if (moduleData.error || moduleData.assemblyError) {
                                        highlightClasses = 'shadow-[0_0_6px_4px_rgba(239,68,68,0.7)]';
                                    } else if (moduleData.isSelected) {
                                        highlightClasses = 'shadow-[0_0_6px_4px_rgba(34,197,94,0.7)]';
                                    } else {
                                        highlightClasses = 'shadow-md';
                                    }
                                    ...
                                    return (
                                        <div 
                                            ...
                                            className={`p-2 border border-slate-200 rounded-lg w-48 sm:w-52 md:w-56 flex-shrink-0 flex flex-col justify-between focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-300 bg-white ${highlightClasses}`} 
                                        ...
                                        <div>
                                            ...
                                            {canSelect && ( <div className="flex items-center space-x-2 p-2 mb-2 rounded-md bg-slate-50 cursor-pointer hover:bg-slate-200 transition-colors" onClick={(e) => { e.stopPropagation(); handleModuleSelect(stageIndex, moduleIndex); }} > <input type="radio" name={`stage-select-${stageIndex}`} id={`module-select-${moduleData.id}`} checked={moduleData.isSelected} onChange={() => handleModuleSelect(stageIndex, moduleIndex)} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500 cursor-pointer" /> <label htmlFor={`module-select-${moduleData.id}`} className={`text-sm font-semibold cursor-pointer ${moduleData.isSelected ? 'text-green-700' : 'text-gray-700'}`} > {moduleData.isSelected ? '✓ Ведущий' : 'Сделать ведущим'} </label> </div> )}

                                            {moduleData.type === GearType.Gear && <GearModuleDisplay onParamClick={handleParamClick} moduleData={moduleData} stageIndex={stageIndex} moduleIndex={moduleIndex} onInputChange={handleInputChange} isExpanded={isStageExpanded}/>}
                                            {moduleData.type === GearType.Chain && <ChainModuleDisplay onParamClick={handleParamClick} moduleData={moduleData} stageIndex={stageIndex} moduleIndex={moduleIndex} onInputChange={handleInputChange} isExpanded={isStageExpanded}/>}
                                            {moduleData.type === GearType.Planetary && <PlanetaryModuleDisplay onParamClick={handleParamClick} moduleData={moduleData} stageIndex={stageIndex} moduleIndex={moduleIndex} onInputChange={handleInputChange} onSelectChange={handleSelectChange} isExpanded={isStageExpanded}/>}
                                            {moduleData.type === GearType.ToothedBelt && <ToothedBeltModuleDisplay onParamClick={handleParamClick} moduleData={moduleData} stageIndex={stageIndex} moduleIndex={moduleIndex} onInputChange={handleInputChange} isExpanded={isStageExpanded}/>}
                                            {moduleData.type === GearType.Belt && <BeltModuleDisplay onParamClick={handleParamClick} moduleData={moduleData} stageIndex={stageIndex} moduleIndex={moduleIndex} onInputChange={handleInputChange} isExpanded={isStageExpanded}/>}
                                            {moduleData.type === GearType.Bevel && <BevelGearModuleDisplay onParamClick={handleParamClick} moduleData={moduleData} stageIndex={stageIndex} moduleIndex={moduleIndex} onInputChange={handleInputChange} onSelectChange={handleSelectChange} isExpanded={isStageExpanded}/>}
                                            {moduleData.type === GearType.Worm && <WormGearModuleDisplay onParamClick={handleParamClick} moduleData={moduleData} stageIndex={stageIndex} moduleIndex={moduleIndex} onInputChange={handleInputChange} onSelectChange={handleSelectChange} isExpanded={isStageExpanded}/>}
                                ...
                                <div className="flex-shrink-0 flex items-center justify-center min-w-[120px]">
                                    <Button onClick={() => addGearVariant(stageIndex)} variant="secondary" className="text-sm px-3 py-1.5 flex items-center h-full" disabled={isAddVariantDisabled} title={isAddVariantDisabled ? "Планетарная, коническая или червячная передача должна быть единственной на ступени." : "Добавить вариант передачи"}><PlusIcon />Добавить</Button>
                                </div>
                                </div>
                            </div>
                 ...
                 <div className="mt-4 flex justify-end">
                    <Button onClick={onResetConfiguration} variant="secondary" className="text-sm">
                        Сбросить все
                    </Button>
                </div>
            </div>
            ...
            <div className={`mt-6 flex flex-col md:flex-row md:flex-wrap ${isSchemeBuilt ? 'justify-between' : 'justify-end'} items-center gap-3`}>
                {isSchemeBuilt && (
                    <Button 
                        ...
                        variant="primary"
                        className="flex-grow w-full md:w-auto md:flex-grow-0 order-2 md:order-1"
                ...
                <Button 
                    ...
                    variant="primary" 
                    className="flex-grow w-full md:w-auto md:flex-grow-0 order-1 md:order-2"
