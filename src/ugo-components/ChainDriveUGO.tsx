...
interface ChainDriveUGOProps {
  width: number;
  height: number;
  z1: number; // Число зубьев ведущей звездочки
  z2: number; // Число зубьев ведомой звездочки
  u: number;  // Передаточное число
  strokeColor?: string;
}

export const ChainDriveUGO: React.FC<ChainDriveUGOProps> = ({
  width,
  height,
  z1,
  z2,
  ...
  const CHAIN_LINE_STROKE_WIDTH = 2;
  ...
  const chainLineY1 = yRect1Top;
  const chainLineY2 = yRect2Top + H2;
    ...
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} xmlns="http://www.w3.org/2000/svg">
      ...
      <g stroke={strokeColor} strokeWidth={RECT_STROKE_WIDTH} fill="none">
        ...
        <line
          ...
          x2={xRectCenter}
          y2={chainLineY2}
          strokeWidth={CHAIN_LINE_STROKE_WIDTH}
          strokeDasharray="4 2"
        />
